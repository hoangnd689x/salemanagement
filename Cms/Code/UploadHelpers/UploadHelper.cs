...
namespace Cms.Code.UploadHelpers
{
    public static class UploadHelper
    {
        ...
        public static async Task<string> UploadFile(ICollection<IFormFile> files)
        {
            ...
            var uploadsCMS = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "Resources/Images");
            var uploadsWeb = uploadsCMS.Replace("Cms", "Web");

            if (!Directory.Exists(uploadsCMS))
                Directory.CreateDirectory(uploadsCMS);
            var filename = StringHelper.StringHelpers.GenerateRandom(10);
            foreach (var file in files)
            {
                if (file.Length > 0)
                {
                    using (var fileStream = new FileStream(Path.Combine(uploadsCMS, file.FileName), FileMode.Create))
                    {
                        await file.CopyToAsync(fileStream);
                    }

                    using (var fileStream = new FileStream(Path.Combine(uploadsWeb, file.FileName), FileMode.Create))
                    {
                        await file.CopyToAsync(fileStream);
                    }
                }
